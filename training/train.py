    ...
    for epoch in range(start_epoch, CONFIG["num_epochs"]):
        ...
        def mid_epoch_save(step, ep=epoch, bvl_ref=lambda: best_val_loss):
            if step % CONFIG["save_every_n_steps"] == 0:
                save_checkpoint(latest_path, model, optimizer, scheduler, ep, step, bvl_ref())

        train_loss, global_step = run_epoch(
            model, train_loader, optimizer, scheduler, loss_fn, tok.pad_id, device,
        ...
        val_loss, _ = run_epoch(
            model, val_loader, optimizer, scheduler, loss_fn, tok.pad_id, device,
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(best_path, model, optimizer, scheduler, epoch, global_step, best_val_loss)
